...
class BriefingDeltaGenerator:
    ...
    async def generate_delta(
        ...
        new_sections = await self._build_sections_from_state(state)
    ...
    def _build_sections_from_state(self, state: Any) -> List[Dict[str, Any]]:
    ...
    def _compare_sections(
        ...
        Args:
            current_sections: Current briefing sections
            new_sections: New sections from intelligence state
        ...
        current_by_type = {s.section_type: s for s in current_sections}
        new_by_type = {s.get("section_type"): s for s in new_sections}
        ...
        sections_added = []
        sections_updated = []
        sections_removed = list(removed_types)

        # Check added sections
        for sec_type in added_types:
            new_sec = new_by_type[sec_type]
            sections_added.append({
                "section_type": sec_type,
                "title": new_sec.get("title", sec_type),
                "content": new_sec.get("content", ""),
                "source_uris": new_sec.get("source_uris", []),
                "confidence": new_sec.get("confidence", 0.7),
            })

        # Check updated sections (content changed)
        for sec_type in common_types:
            current_sec = current_by_type[sec_type]
            new_sec = new_by_type[sec_type]

            # Compare content (simplified - could use diff)
            if current_sec.content != new_sec.get("content", ""):
                sections_updated.append({
                    "section_type": sec_type,
                    "title": new_sec.get("title", current_sec.title),
                    "content": new_sec.get("content", ""),
                    "source_uris": new_sec.get("source_uris", []),
                    "confidence": new_sec.get("confidence", current_sec.confidence),
                })
        ...
        # Count new anomalies
        anomaly_sections = [s for s in new_sections if s.get("section_type") == "anomaly_alerts"]
        new_anomaly_count = sum(len(s.get("content", "").split("\n")) for s in anomaly_sections)
    ...
    async def _write_delta_to_minio(
        ...
        """Write delta to MinIO."""
        bucket = f"stratops-briefings-{tenant_id}"
