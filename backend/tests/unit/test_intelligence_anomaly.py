...
class TestAnomalyDetectorNode:
    ...
    async def test_isolation_forest_training(self, anomaly_detector) -> None:
        ...
        # Mock training data
        with mock.patch.object(anomaly_detector, "_extract_training_features") as mock_extract:
    ...
    async def test_generate_recommendation(self, anomaly_detector) -> None:
        ...
        mock_response.json = mock.AsyncMock(
            return_value=[{"summaries": ["Investigate pricing volatility immediately."]}]
        )
    ...
    @pytest.mark.asyncio
    async def test_write_anomalies_to_minio(self, anomaly_detector, mock_minio_client) -> None:
        ...
        uris = await anomaly_detector._write_anomalies_to_minio(tenant_id, trace_id, anomalies)
    ...
    @pytest.mark.asyncio
    async def test_full_call_pointer_only(self, anomaly_detector, mock_minio_client) -> None:
        ...
        with mock.patch.object(anomaly_detector, "_extract_entity_features") as mock_extract:
    ...
    @pytest.mark.asyncio
    async def test_detect_anomalies_with_mock_model(self, anomaly_detector) -> None:
    ...
    @pytest.mark.asyncio
    async def test_write_anomalies_empty_list(self, anomaly_detector, mock_minio_client) -> None:
        """Test writing empty anomalies list."""
        uris = await anomaly_detector._write_anomalies_to_minio("001", "trace-001", [])
        assert uris == []
