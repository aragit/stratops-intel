...
from backend.alerts.rules import AlertRule, AlertRuleEngine
from backend.db.neo4j_client import Neo4jClient
from backend.intelligence.agents.anomaly import AnomalyDetectorNode
from backend.intelligence.agents.composer import BriefingComposerNode
...
class TestFullPipeline:
    ...
    async def test_full_pipeline(
        ...
        mock_minio_briefing.upload = mock.AsyncMock(
            return_value="s3://stratops-briefings-test/briefing-123/v1.md"
        )
        ...
        with mock.patch.object(
            AlertRuleEngine, "_extract_metrics", return_value={"Apple Inc.": 0.2, "Microsoft": 0.1}
        ):
            alerts = await alert_engine.evaluate(result_state, [pricing_rule, anomaly_rule])
        ...
        # ================================
        _slack_config = SlackChannelConfig(
            webhook_url="https://hooks.slack.com/test",
            username="TestBot",
        )
        _email_config = EmailChannelConfig(
...
class TestRLSEnforcement:
    ...
    async def test_tenant_isolation(self, neo4j_client):
        ...
        result = await neo4j_client.run(
            "MATCH (b:Briefing {tenant_id: $tid}) RETURN count(b)", {"tid": tenant_b}
        )
